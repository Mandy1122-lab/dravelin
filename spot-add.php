...
<body class="font_set">
    ...
    if (isset($_POST["Insert"])) {
        ...
        $s_add = mysqli_real_escape_string($conn, $_POST["s_add"]);
        $s_info = mysqli_real_escape_string($conn, $_POST["s_info"]);
        $s_intro = mysqli_real_escape_string($conn, $_POST["s_intro"]);
        $s_photo = mysqli_real_escape_string($conn, $_POST["s_photo"]);
        $s_pic = mysqli_real_escape_string($conn, $_POST["s_pic"]); 
        $lat_lon = mysqli_real_escape_string($conn, $_POST["lat_lon"]);
        $c_id = intval($_POST["c_id"]);
        $frame = mysqli_real_escape_string($conn, $_POST["frame"]);
        
        $sql = "INSERT INTO spot (s_name, s_add, s_info, s_intro, s_photo, s_pic, lat_lon, frame) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "ssssssss", $s_name, $s_add, $s_info, $s_intro, $s_photo, $s_pic, $lat_lon, $frame);
        if (mysqli_stmt_execute($stmt)) {
            $s_id = mysqli_insert_id($conn);

            $sql = "INSERT INTO cspot (c_id, s_id) VALUES (?, ?)";
            $stmt = mysqli_prepare($conn, $sql);
            mysqli_stmt_bind_param($stmt, "ii", $c_id, $s_id);
            if (mysqli_stmt_execute($stmt)) {
                mysqli_close($conn);
            ...
            <div>
                ...
                <form action="spot-add.php" method="post">
                    ...
                    <div class="col">
                        ...
                        <input type="text" name="frame" class="form-control" placeholder="請輸入 Google 地圖 HTML">
